...
"""
Batch-generate azimuth/elevation flight-pattern PNG plots for every archived
scenario in `output/scenarios/`, saving one `pattern_<scenario>.png` per scenario
folder into `notebooks/`.
...
uncorrected path if available), then saves it to `notebooks/pattern_<scenario>.png`.
