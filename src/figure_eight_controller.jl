...
zero in the middle of the pattern and `lift` at the lobes, so the pattern is
raised where that buys clearance instead of rigidly. Raising the whole lobe,
top included, is a vertical translation of the part of the path that turns
hardest, which is why this profile is nearly free.

`mode` selects the units of `az_full` and `az_blend`:

  * `"azimuth"` — zero inside `|az| = az_full - az_blend`, `lift` beyond
    `az_full`, smoothstep between, both in DEGREES.
...
function lobe_lift(az, el; lift = 0.0, mode = "azimuth", az_full = 10.0,
    ...
    lift == 0 && return out
    mode == "azimuth" || mode == "azimuth_frac" ||
        throw(ArgumentError("unknown lobe-lift mode \"$mode\"; use \"azimuth\" or \
                             \"azimuth_frac\""))
