...
"""
    winch_kv(v_wind) -> Float64

Wind-speed-dependent `kv` for WinchControllers.jl's `v_set = kv * sqrt(force)`
reel-out law, linearly interpolated between the rows of
`data/winch_kv_table.yaml`. Clamped to the lowest/highest identified wind
speed outside that range, never extrapolated. Overrides the flat `kv` of
`data/wc_settings.yaml`; the caller assigns the result to `WCSettings.kv`.
"""
function winch_kv(v_wind)
    raw = YAML.load_file(joinpath(skc_data_path(), "winch_kv_table.yaml"))
